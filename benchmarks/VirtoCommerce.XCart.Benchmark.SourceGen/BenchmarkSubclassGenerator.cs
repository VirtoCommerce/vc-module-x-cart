...
public sealed class BenchmarkSubclassGenerator : IIncrementalGenerator
{
    ...
    private static void Execute(SourceProductionContext context, Compilation compilation)
    {
        ...
        // No [assembly: BenchmarkSetup] → emit nothing, and do NOT raise a diagnostic. Two compilations
        // legitimately reach here without the attribute: Core itself (it references this generator only
        // so the DLL gets packed into its nupkg) and BenchmarkDotNet's out-of-process autogenerated child
        // project (it references Core with the analyzer, but its subclasses come from the referenced
        // runner assembly). An error here breaks both.
