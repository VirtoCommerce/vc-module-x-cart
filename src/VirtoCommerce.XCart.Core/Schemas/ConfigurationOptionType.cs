...
public class ConfigurationOptionType : ExtendableGraphType<ExpProductConfigurationOption>
{
    ...
    {
        Field(x => x.Id).Description("The unique identifier of the configuration option");
        Field(x => x.Text, nullable: true).Description("The text of the Text-type configuration option");
        Field(x => x.Quantity).Description("The quantity of the configuration option");

        var productField = new FieldType
        {
            Name = "product",
            Type = GraphTypeExtensionHelper.GetActualType<ProductType>(),
            Resolver = new FuncFieldResolver<ExpProductConfigurationOption, IDataLoaderResult<ExpProduct>>(context =>
            {
