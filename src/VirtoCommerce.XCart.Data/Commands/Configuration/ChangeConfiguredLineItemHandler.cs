...
public class ChangeConfiguredLineItemHandler(
    ...
    : ConfigurationCommandHandler<ChangeConfiguredLineItemCommand>(configuredLineItemContainerService, cartProductService, fileUploadService)
{
    public override async Task<ExpConfigurationLineItem> Handle(ChangeConfiguredLineItemCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var section in request.ConfigurationSections)
        {
            ...
            {
                var configurationItem = request.ConfigurationItems.FirstOrDefault(x => x.SectionId == section.SectionId);
                var files = await CreateFiles(section, configurationItem);
